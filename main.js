...
//     let pc_main_page = document.querySelectorAll(".pc_main_page");
//     pc_main_page.forEach(function (element) {
//         element.remove();
//     });
// }
	  ...
	  */
        ...
        function lazyLoadOthers() {
            var hasAutoHeightSections =
                $(AUTO_HEIGHT_SEL)[0] || (isResponsiveMode() && $(AUTO_HEIGHT_RESPONSIVE_SEL)[0]); //quitting when it doesn't apply
        ...
        function scrollPage(section, callback, isMovementUp) {
            ...
            if (
                ...
                (getOptions().scrollBar && getScrollTop() === v.dtop && !hasClass(element, AUTO_HEIGHT))
            ) {
                return;
            }
		...
		*/
        ...
        var scrollOverflowHandler = {
            ...
            isScrolled: function isScrolled(direction, el) {
                ...
                var isBottomReached =
                    direction === "down" &&
                    scrollableItem.scrollHeight <= Math.ceil(scrollableItem.offsetHeight + positionY) + ie11offset;
            ...
            if (getState().isBeyondFullpage) {
                if (!pauseScroll) {
                    ...
                    var shouldSetFixedPosition =
                        ...
                        (!keyframeTime("isNewKeyframe", "beyondFullpage") || !wheelDataHandler.isAccelerating());
                    var scrollSettings;

                    if (shouldSetFixedPosition) {
                        scrollSettings = getScrollSettings(
                            getLast(getState().sections).item.offsetTop + getLast(getState().sections).item.offsetHeight
                        );
                        scrollSettings.element.scrollTo(0, scrollSettings.options);
                        setState({
                            isAboutToScrollToFullPage: false,
	  ...
	  */
        ...
        function touchMoveHandler(e) {
            ...
            if (isReallyTouch(e)) {
                ...
                var isVerticalMovementEnough =
                    Math.abs(touchStartY - touchEndY) > (win.innerHeight / 100) * getOptions().touchSensitivity;
                var isHorizontalMovementEnough =
                    Math.abs(touchStartX - touchEndX) > (getWindowWidth() / 100) * getOptions().touchSensitivity;
        ...
        function keydownHandler(e) {
            ...
            var canScrollWithKeyboard =
                getOptions().autoScrolling || getOptions().fitToSection || isPressingHorizontalArrows; //tab?
        ...
        function onTab(e) {
            ...
            if (activeElement) {
                if (
                    closest(activeElement, SECTION_ACTIVE_SEL + "," + SECTION_ACTIVE_SEL + " " + SLIDE_ACTIVE_SEL) ==
                    null
                ) {
                ...
                if (getOptions().fitToSection && state.canScroll) {
                    ...
                    g_scrollId2 = setTimeout(function () {
                        var fixedSections = state.sections.filter(function (section) {
                            ...
                            return (
                                Math.round(sectionValues.bottom) === Math.round(getWindowHeight()) ||
        ...
        function destroyStructure() {
            ...
            $("img[data-src], source[data-src], audio[data-src], iframe[data-src]", getContainer()).forEach(function (
                item
            ) {
                setSrc(item, "src");
            });
